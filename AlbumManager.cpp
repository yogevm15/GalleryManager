...
void openirfan(std::string path) {
	currProccessName = "i_view64.exe";
	STARTUPINFO si;
	...
	std::string realpath(fullFilename);
	std::string  command = "i_view64.exe " + realpath;
...
void openPaint(std::string path) {
	currProccessName = "mspaint.exe";
	STARTUPINFO si;
	PROCESS_INFORMATION pi;
	...
	std::string  command = "mspaint " + path;
	// Start the child process. 
	if (!CreateProcess(NULL,   // No module name (use command line)
		(LPSTR)command.c_str(),        // Command line
		NULL,           // Process handle not inheritable
...
BOOL WINAPI consoleHandler(DWORD signal) {
	...
	if (signal == CTRL_C_EVENT) {
		if (currProccessName != "") {
			killProcessByName(currProccessName.c_str());
		}
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Albums count: " << m_dataAccess.countAlbumsOwnedOfUser(user) <<std::endl;
}
